# Holdout 阶段 -- 横截面因子研究的留存验证

## 1. 阶段定义与核心目的

### 1.1 阶段定义

**Holdout（留存验证）** 是横截面因子研究中，在完全未参与任何设计、调参或优化环节的最终时间窗口上，对策略进行最后一次独立验证的阶段。

本阶段的核心定位：

> **这不是又一次回测，而是策略投入实盘前的"盲测"——只有一次机会，结果即为终审。**
...
Holdout 阶段需要对前序所有阶段的核心指标进行终审，确认没有显著退化。
...
过拟合是 Holdout 阶段最需要警惕的问题。以下是基于指标退化的过拟合信号分级标准：
