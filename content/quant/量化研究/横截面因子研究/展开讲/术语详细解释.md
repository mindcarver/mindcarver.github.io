...
- `factor_panel` / `factor_manifest` / `factor_contract`
  为什么要有：CSF SignalReady 需要把真实 factor panel、因子身份和字段语义一起冻结下来。
  不写清会怎样：下游可能拿到了文件，却不知道每列到底是什么意思，也不知道是不是在消费同一个 factor 实体。
- `baseline-only`
  为什么要有：第一版 signal_ready 只冻结 baseline signal，不把 full search grid 一次性塞进去。
  不写清会怎样：Signal 阶段和 Train 阶段会重新混在一起，参数空间也会失控。
...
- `holdout_test_compare` / `holdout_portfolio_compare`
  为什么要有：把 holdout 和 TestEvidence / BacktestReady 放在同一坐标系下比较。
...
人话解释：事后看起来最好的持有期。对 CSF 横截面因子路线来说，这是需要警惕的非主线口径。

例子：
你测了 `1d、3d、5d、10d`，发现 `5d` 最好，于是只汇报 `5d`。
这就是典型的事后最优口径，容易把研究带偏。

在 `qros-csf-*` 阶段里，不应产出 `best_h`、预测 horizon 或单资产命中率语义。
...
人话解释：CSF SignalReady 真正落盘的 `date x asset` 因子值面板。

例子：
不是只说“20 日动量”，而是生成 `factor_panel.parquet`，让下游能在同一张横截面面板上复用同一个因子。

### `factor_manifest`

人话解释：记录 factor 身份、结构、角色、输入字段和依赖关系的清单。
...
所以它更像“当前 SignalReady 真正交付了哪个 factor 实体”。

### `factor_coverage_report`

人话解释：这个 factor 在多少 `date x asset` 样本上真的有有效值。

例子：
如果 `MOM_20D` 在 10000 个 `date x asset` 样本里只有 3500 个有值，
那 coverage 只有 35%，这通常就需要在 `factor_coverage_report.parquet` 或 gate decision 里解释清楚。

### `factor_contract` / `factor_field_dictionary`

人话解释：一个解释 factor 是谁，一个解释 factor 文件里的列是什么。

例子：

- `factor_contract.md` 更像“这个 factor 的合同”
- `factor_field_dictionary.md` 更像“这张 parquet 里的每列都代表什么”

二者一起用，才能让下游既知道身份，又知道 schema。

### `csf_selected_variants_test`

人话解释：在 CSF TestEvidence 结束时，正式冻结下来、允许进入 backtest 的候选 variant 清单。

例子：
不是“感觉这几个币看起来更靠谱”，而是明确落成：

- `csf_selected_variants_test.csv`

### `csf_test_contract`

人话解释：TestEvidence 给 BacktestReady 的正式证据合同。

例子：
它应明确：

- 哪些 variant 通过 formal evidence gate
- 这些 variant 的证据口径是什么
- BacktestReady 只能消费哪一版冻结候选

这就相当于说：

> Backtest 之后不能再自己重选 variant，也不能把 TestEvidence 改写成交易层胜利宣言。
...
人话解释：比较 holdout 和历史测试证据、历史回测组合结果的一致性。
