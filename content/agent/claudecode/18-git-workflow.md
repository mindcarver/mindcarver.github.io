...
被列入黑名单的命令有共同特征：都可能导致工作成果的不可逆丢失。`push --force` 会覆盖远程历史，`reset --hard` 会丢弃所有未提交的修改，`checkout .` 会还原所有本地文件变动，`clean -f` 会删除未跟踪的文件，`branch -D` 会强制删除分支。
...
最后一句话是整条规则的点睛之笔："If a hook fails, investigate and fix the underlying issue." Claude Code 被要求不是绕过失败的 hook，而是追根溯源修复问题。当代码无法通过质量关卡时，正确做法是修复代码，而非移除关卡。
...
Claude Code 的 commit 创建流程被设计为一个三阶段的管线，每一阶段都有明确的输入、处理和输出。
...
这条规则被标记为 CRITICAL。拆解 `git commit --amend` 的工作原理就能明白为什么：
...
Co-Authored-By 是 git 社区公认的多作者标记方式。Claude Code 在 commit message 末尾添加此标记，声明这个 commit 是人机协作的结果。这对代码审计和贡献者归属都有实际意义。
...
PR 的创建使用 `gh` CLI 工具。文档明确要求 "Use the gh command via the Bash tool for ALL GitHub-related tasks including working with issues, pull requests, checks, and releases." [source: tool-description-bash-git-commit-and-pr-creation-instructions]
...
每一条规则都对应着一个真实的事故场景，不是理论上的风险假设。这套协议的设计思路是：在每一个可能出错的环节设置防线，让 AI agent 的 git 操作在效率上接近人类开发者，但在安全性上远超随意执行命令的自动化脚本。
