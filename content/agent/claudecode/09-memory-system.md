...
Claude Code 的记忆不是一块无差别的存储空间，而是按类型划分，每种类型有各自的用途边界和写入规则。

### 2.1 User Memory -- 用户画像

User memory 存储的是关于用户本身的信息，而非关于项目或代码的信息。系统提示词对它的定义是：
...
这段定义的关键在于 "tailor your future behavior"——用户记忆的目的是让 agent 在后续交互中调整行为。比如面对一位资深工程师和一位刚入门的学生，agent 的沟通深度和辅助方式应该不同。
...
Feedback memory 存储用户对 agent 工作方式的指导，包括"应该避免什么"和"继续保持什么"。系统提示词的描述是：
...
这个类型解决了一个微妙的问题：如果只记录纠错信息而忽略正面反馈，agent 会逐渐变得过度保守。提示词明确指出：
...
个人反馈不能静默覆盖团队共享的工作方式规范。当个人偏好与团队规范冲突时，系统要求显式标注这种覆盖关系。
...
Project memory 的内容是代码库的结构性知识：代码模式、风格约定、常见问题、架构决策。它不是用户说的，也不是用户纠正的，而是 agent 在工作过程中自主发现的。

### 2.4 记忆类型的注入方式

在对话过程中，记忆文件的内容通过特定的模板注入到上下文中。私有记忆使用简单的文本注入：
...
在同一次对话中，一旦某条记忆已经被附加过，后续查询不再重复附加。这既节省了 context window 空间，也避免了重复信息造成的干扰。
...
四个步骤从粗到细扫描：先看目录结构，再读索引文件，然后浏览已有的主题文件，最后检查子目录中的近期记录。这个顺序确保 agent 在写入任何新记忆之前，已经知道"已经有什么"。
...
三种来源并非都要扫描。Gather 阶段的设计允许 agent 根据需要选择性使用来源，日志和记忆漂移检测是主要来源，抄本搜索是按需补充。
...
当新信息与已有记忆属于同一主题时，合并到现有文件中，而非创建新文件。这防止了记忆文件的碎片化，如果每次 Dream 都创建新文件而非更新旧文件，记忆目录会膨胀到无法管理。
...
记忆不是只增不减的。当新发现与旧记忆矛盾时，必须直接修正源头，而非在旁边追加一条"更正"。这样可以避免记忆系统中的自相矛盾，如果不主动删除被推翻的事实，后续会话的 agent 在面对两条相互矛盾的记忆时会陷入困惑。
...
除了跨会话的 Dream 整合流程，Claude Code 还有一套在会话进行中实时更新的记忆机制，session memory。这通过一个独立的 agent 在对话进行时持续工作。
...
具体的文件路径、函数名、错误信息、精确命令，这些是恢复工作上下文最需要的信息。模糊的描述如"修改了一些文件"在这里是不合格的。
...
"compaction" 是 Claude Code 在长对话中压缩上下文的机制。当上下文被压缩后，session notes 的 "Current State" 部分是 agent 恢复工作连续性的关键锚点。
...
**Errors & Corrections 的独立地位**：错误和修正常被混在叙述中，但这里获得了独立章节。这说明 Claude Code 团队在实践中发现，"什么方案失败了"与"什么方案成功了"同样重要。失败的尝试如果不被记录，后续会话的 agent 可能会重复踩坑。

**Worklog 的定位**：工作日志是模板的最后一个章节，定位为"very terse summary"，每一步只用极简的措辞概括。这与前几个章节的信息密度要求形成互补：前面的章节追求详细和具体，Worklog 追求时间线的完整性而非单条记录的深度。
...
这个定义的核心是"read back"：记忆中毒的危害不在于写入动作本身，而在于写入的内容在未来被读回时会被 agent 当作合法指令执行。具体的攻击模式包括：
...
另外，这个 ALLOW 规则还划清了记忆写入与另外两个安全规则的边界：它不是 Self-Modification（agent 修改自身配置），也不是 Irreversible Local Destruction（不可逆的本地文件销毁）。记忆文件是系统设计中有意为之的持久化机制，正常的读写是预期行为。
...
这套系统的设计并不追求最大化的信息保留，而是在信息保留与信息噪音之间寻找工程最优解。记忆的价值不在于记住了一切，而在于需要的时候能精准地回忆起正确的信息。
