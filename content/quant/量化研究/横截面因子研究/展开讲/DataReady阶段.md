...
DataReady 的英文字段主要在定义“同一张研究世界”。它们不是数据工程细节，而是后续所有因子、训练和验证共同依赖的底座。

| 字段 | 人话含义 | 为什么要用它 |
| --- | --- | --- |
| `date x asset` | 每条样本是一条“某时点、某资产”的记录。 | 横截面研究必须在同一时点比较不同资产，主键不清就无法比较。 |
| `date_key` | 时间主键。 | 告诉下游每行属于哪个时点，避免同一时间被不同代码解释成不同窗口。 |
| `asset_key` | 资产主键。 | 告诉下游每行属于哪个资产，避免 symbol、coin id、合约名混用。 |
| `panel_frequency` | 面板频率，如 `1h`、`4h`、`1d`。 | 因子、收益和调仓都依赖频率，频率不清会导致信号错位。 |
| `timezone` | 时间戳解释时区，通常统一为 `UTC`。 | 防止有人按北京时间、有人按 UTC 对齐，造成隐性 lookahead 或错位。 |
| `timestamp_semantics` | 时间戳语义，如 `close_time` 或 bar open。 | 同一个时间戳到底代表开盘还是收盘，决定信号何时可见。 |
| `asset_universe_membership` | 某时点某资产是否属于研究 universe。 | 区分“本来不在研究对象里”和“在对象里但数据不可用”。 |
| `eligibility_base_mask` | 某时点某资产是否具备基础研究资格。 | 只记录基础可研究性，不能混入具体因子好坏。 |
| `eligible` | 该观测满足基础研究条件。 | 下游可以统一跳过不可研究样本，而不是各阶段自己过滤。 |
| `cross_section_coverage` | 每个时点横截面覆盖情况。 | 告诉后续阶段某些时点截面是否太薄，避免在塌掉的截面上做证据。 |
| `taxonomy_contract` | 资产分类体系合同。 | 后续 `group_neutral`、分组暴露和分类审计都依赖它。 |
| `shared_feature_base` | 共享基础特征层。 | 统一 `log_market_cap`、`beta_30d` 等通用字段，避免每个阶段各算一版。 |
| `log_market_cap` | 市值取对数后的字段。 | 市值分布很偏，取对数后更适合中性化和风险控制。 |
| `beta_30d` | 过去 30 天估计的市场 beta。 | 后续可用于市场暴露控制，避免把 beta 暴露误当 alpha。 |
| `run_manifest.json` | 运行清单。 | 记录输入、程序、版本和 replay 信息，保证 DataReady 可复现。 |
| `field_dictionary.md` | 字段字典。 | 逐字段解释 schema，避免下游误读列含义。 |
