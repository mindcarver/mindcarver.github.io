...
在 Claude Code 的系统提示词中，有一份名为 "Agent Design Patterns" 的技能文档。它的定位不是给终端用户看的操作手册，而是 Anthropic 工程师在构建自己的 Agent 产品（即 Claude Code 本身）时沉淀下来的设计决策记录。这份文档覆盖了工具面设计、上下文管理、缓存策略、多 Agent 协作等核心问题 [source: skill-agent-design-patterns.md]。

这意味着我们面对的不是理论推演，而是经过大规模工程验证的实战经验。Claude Code 每天在真实的代码库中被成千上万的开发者使用，它内部的每一个设计决策都经过了成本、延迟、可靠性的三重考验。本文基于这份指南以及 Claude Code 中 Agent 相关的多个系统提示词，梳理 Anthropic 的 Agent 设计方法论。

文章会依次探讨：工具面设计的核心权衡、Anthropic 提供的六种工具矩阵、Programmatic Tool Calling 的组合调用机制、按需加载的工具发现系统、长运行 Agent 的上下文生存策略、Agent 特定的缓存变通方案、三种多 Agent 协作模式的对比，以及如何用 Agent Creation Architect 来设计自定义 Agent。
...
当把某个操作提升为专用工具时，框架获得了一个带有类型化参数的动作钩子，可以拦截、门控、渲染或审计。Anthropic 明确了四个提升时机：
...
**调度（Scheduling）**：只读工具如 `glob` 和 `grep` 可以被标记为并行安全（parallel-safe）。当同样的操作通过 Bash 执行时，框架无法区分并行安全的 `grep` 和不安全的 `git push`，只能全部串行化 [source: skill-agent-design-patterns.md]。

### 经验法则

Anthropic 给出的规则很简单：先用 Bash 获得广度，当需要门控、渲染、审计或并行化某个操作时，再将其提升为专用工具 [source: skill-agent-design-patterns.md]。这种渐进式的设计策略避免了过早抽象的陷阱。

---

## 三、Anthropic 提供的六种工具矩阵

Anthropic 为 Claude API 提供了六种开箱即用的工具，分为 Client-side 和 Server-side 两大阵营 [source: skill-agent-design-patterns.md]。
...
这种 Client/Server 分离的设计哲学值得关注。Bash 和 Text Editor 被放在 Client 端，意味着 Anthropic 认为文件操作和命令执行必须由开发者完全控制。而 Code Execution 和 Web Search 放在 Server 端，说明 Anthropic 愿意为计算密集型和网络访问型任务提供托管能力。Computer Use 的独特之处在于它可以是任意一侧——如果你自己运行环境就是 Client-side，如果用 Anthropic 托管的环境就是 Server-side。
...
这是一种根本性的架构转变。传统模式是"工具结果进入上下文，Agent 在上下文中推理"；PTC 模式是"Agent 编写代码，代码在沙箱中自主执行多步操作，只有最终结论回传上下文"。

### 适用场景

PTC 适用于有大量顺序工具调用的场景，或者中间结果很大、希望在进入上下文窗口之前先过滤的情况。Token 成本按最终输出计费，而非中间结果 [source: skill-agent-design-patterns.md]。

这在实践中意味着：如果一个任务需要读取十个文件、在每个文件中搜索特定模式、汇总结果，传统模式会把十个文件的内容都塞进上下文窗口；而 PTC 模式下，Claude 写一个脚本来完成所有读取和过滤，只把汇总表返回给上下文。
...
当有许多工具可用但每次请求只有少数几个相关时，Tool Search 允许 Claude 搜索工具集并只加载相关的 Schema。关键细节是：工具定义是追加（append）而非替换的——这保护了已有的缓存前缀 [source: skill-agent-design-patterns.md]。

### Skills

Skills 是任务特定的指令，Claude 只在相关时才加载。每个 Skill 是一个文件夹，包含一个 `SKILL.md` 文件。Skill 的描述（description）默认在上下文中，但完整的文件内容只在任务需要时 Claude 才会读取 [source: skill-agent-design-patterns.md]。

这两种模式共享同一个设计原则：保持固定上下文尽可能小，按需加载细节。这不仅是成本优化，更是信号质量优化。上下文中塞满无关的工具定义会稀释 Agent 对真正相关信息的注意力。
...
随着对话轮次增加，上下文会变得陈旧——旧的工具结果、已完成的思考块不再有价值。Context Editing 根据可配置的阈值清除工具结果和思考块。它保持对话记录精简，而不使用摘要。这是一种"修剪"策略：移除明确无用的内容，保留仍有价值的部分 [source: skill-agent-design-patterns.md]。

### Compaction（压缩）

当对话可能达到或超过上下文窗口限制时，Compaction 在服务端将较早的上下文总结为一个压缩块。这是一种"浓缩"策略：将大量历史信息压缩为更紧凑的表示 [source: skill-agent-design-patterns.md]。

### Memory（记忆）

当状态必须跨会话持久化（不仅限于单次对话内）时，Claude 在一个记忆目录中读写文件。这种方式能在进程重启后存活 [source: skill-agent-design-patterns.md]。
...
这三种策略有一个清晰的演进关系。从单轮对话到多轮对话，Context Editing 解决了"对话太长"的问题。当对话远超窗口容量时，Compaction 提供了安全阀。当需要跨会话延续时，Memory 提供了持久化能力。三者构成了一个完整的上下文生命周期管理方案。
...
这三种变通方案共同指向一个设计原则：在 Agent 系统中，保持缓存前缀不变是一等优先级。System Prompt、模型、工具列表——这三者构成了缓存的基础。任何对它们的修改都应该通过"追加而非替换"的方式来规避缓存失效。

---

## 八、多 Agent 协作：Subagent vs Fork vs Swarm 三种模式对比

当单个 Agent 无法胜任复杂任务时，多 Agent 协作成为必然选择。Claude Code 内部实现了三种不同的多 Agent 模式，每种模式针对不同的问题空间。
...
Fork 的核心判断标准是定性的："我还需要这个输出吗？"——而非任务大小。具体来说：
...
**实现任务**：需要超过几个编辑的实现工作适合 Fork。但前提是先完成研究再跳到实现 [source: system-prompt-fork-usage-guidelines.md]。

Fork 的经济性来自缓存共享。因此 Anthropic 明确规定：不要在 Fork 上设置不同的 `model`——不同模型无法复用父 Agent 的缓存 [source: system-prompt-fork-usage-guidelines.md]。
...
Fork 内部使用的 Worker Prompt 很值得研究。它以"STOP. READ THIS FIRST."开头，强调"你不是主 Agent"。规则包括：不要再生成 Subagent、不要对话或问问题、不要添加元评论、直接使用工具、如果修改文件则在报告前提交、工具调用之间不要输出文本、报告限制在 500 字以内、必须以"Scope:"开头 [source: agent-prompt-worker-fork-execution.md]。

这种极简风格反映了 Fork 的设计哲学：Fork 是一个执行单元，不是对话伙伴。

### Swarm：多 Agent 团队协作

Swarm 是通过 TeammateTool 实现的团队协作模式。它创建一个命名的团队，团队成员共享任务列表，通过消息系统协作 [source: tool-description-teammatetool.md]。

Swarm 的工作流程是：创建团队、创建任务、生成团队成员、分配任务、成员完成任务后标记为完成、最后优雅关闭团队 [source: tool-description-teammatetool.md]。

团队成员之间的通信通过 SendMessage 工具完成。消息自动投递，不需要手动检查收件箱。每个成员在完成一个回合后进入空闲状态（idle），这是完全正常的流程——空闲只是意味着在等待输入 [source: tool-description-teammatetool.md]。
...
Claude Code 内置了一个名为 "Agent Creation Architect" 的元 Agent——它的职责是帮助用户设计自定义 Agent。通过研究它的系统提示词，我们可以反推出 Anthropic 认为一个好的 Agent 设计应该包含哪些要素 [source: agent-prompt-agent-creation-architect.md]。
...
最后一条原则值得多说两句。Anthropic 不期望 Agent 是完美无缺的，而是期望 Agent 有能力发现和纠正自己的错误。这种"可恢复性"设计比"无错性"设计更现实。
...
**第一，渐进式设计。** 不是一开始就设计完美的工具面，而是从 Bash 起步获得广度，在遇到安全、渲染、审计、并行化的具体需求时再提升为专用工具。这不是懒惰，而是对过度工程的警惕 [source: skill-agent-design-patterns.md]。

**第二，上下文是最稀缺的资源。** 从 Tool Search 的按需加载、到 Context Editing 的修剪、到 Compaction 的压缩、到 PTC 的"中间结果不进上下文"，所有的设计都在围绕同一个目标：让每一 Token 都承载最高的信息密度。Fork 的存在本身就是为了防止工具噪声污染主 Agent 的上下文 [source: skill-agent-design-patterns.md, system-prompt-fork-usage-guidelines.md]。

**第三，缓存一致性是一等优先级。** System Prompt 追加而非修改、子任务隔离到 Subagent 以避免模型切换、工具 Schema 追加而非替换。这些变通方案共同表明：在 Agent 系统中，缓存失效的代价远大于代码的复杂性 [source: skill-agent-design-patterns.md]。
...
**第五，多模式协作而非一刀切。** Subagent、Fork、Swarm 三种模式各有适用场景，它们的差异不是功能上的高低之分，而是针对不同问题空间的专门化设计。Subagent 擅长独立判断，Fork 擅长并行研究，Swarm 擅长持续协作。一个成熟的 Agent 系统应该能根据任务特征选择合适的协作模式 [source: system-prompt-fork-usage-guidelines.md, tool-description-teammatetool.md]。

这五条原则形成了一个完整的设计框架：以渐进式方法构建工具面、以上下文效率为核心约束、以缓存一致性为系统保障、以"委托执行不委托理解"为协作准则、以多模式选择应对不同复杂度的任务。Anthropic 没有试图提供一个通用的 Agent 框架，而是提供了一套经过实战验证的设计决策——这些决策来自每天服务大量开发者的真实产品，而非实验室中的原型。
