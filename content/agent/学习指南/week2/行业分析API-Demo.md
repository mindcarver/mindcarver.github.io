...
前文已经设计了后端的结构框架，这一篇要把这个框架跑通，做一个完整的、可演示的行业分析 API Demo。

这个 Demo 不只是一个技术验证。它要模拟一个真实的产品场景：用户输入一个行业名称，系统自动完成需求解析、Prompt 模板选择、模型调用、结构化输出、结果保存、报告生成，最终返回一份结构化的行业分析结果和可选的 Markdown 报告。

这是 Week 2 所有内容重点的一次综合实战。你要用到 第 8 篇 的 LLM Client、第 9 篇 的结构化输出、第 10 篇的 Function Calling、第 11 篇 的 Prompt 模板、第 12 篇 的后端架构。把这些能力串起来，形成一个完整的调用链路。

完成今天的学习后，你应该拥有一个可以给别人演示的 AI 行业分析服务——输入行业名称，输出分析结果。这听起来简单，但把所有环节串通、处理各种异常情况、保证输出质量，需要综合运用你这一周建立的所有技能。
...
Prompt 模板的实现可以用 第 11 篇 建立的模板库系统——每个模板有编号、版本、参数占位符、示例输入输出。这样可以追踪哪个模板效果好、哪个需要优化。
...
用 JSON Schema 定义这个结构，然后用 第 9 篇 建立的 JSON Mode 确保模型输出符合这个结构。如果模型输出不符合要求，使用带反馈的重试机制。
...
行业分析 API Demo 是 Week 2 所有知识的综合应用。从输入到输出，每个环节都用到了本周建立的技能。
