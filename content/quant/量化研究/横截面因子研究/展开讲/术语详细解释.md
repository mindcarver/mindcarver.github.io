...
- `param_id`
  为什么要有：current QROS 更强调“下游消费的是哪个已物化的 signal 身份”，而不是只靠自然语言因子名。
  不写清会怎样：Train/Test 会各自引入一版“差不多”的参数组合，结果无法审计。
...
- `param_manifest` / `signal_contract` / `signal_fields_contract`
  为什么要有：current SignalReady 需要把 `param_id`、真实 signal schema 和字段语义一起冻结下来。
  不写清会怎样：下游可能拿到了文件，却不知道每列到底是什么意思，也不知道是不是在消费同一个 `param_id`。
- `baseline-only`
...
人话解释：current signal 层真正被冻结的“身份编号”。
...
这样下游消费时，拿到的是一个明确 identity，而不是一个模糊标签。

### `param_manifest`

人话解释：记录哪些 `param_id` 已经被正式物化出来的清单。
...
所以它更像“当前 SignalReady 真正交付了哪些 signal 身份”。

### `params/`

人话解释：每个 `param_id` 对应的真实 signal timeseries 文件目录。

例子：
目录里可能有：

- `params/MOM_20D_BASE.parquet`
- `params/TURNOVER_SURGE_BASE.parquet`

下游真正消费的不是一句“我们有个动量信号”，
而是这里已经落盘的具体文件。

### `signal_coverage`

人话解释：这些 signal 到底有多少样本真的算出来了。

例子：
如果 `MOM_20D_BASE` 在 10000 个 `date x asset` 样本里只有 3500 个有值，
那 coverage 只有 35%，这通常就需要在 `signal_coverage.md` 里解释清楚。

### `signal_contract` / `signal_fields_contract`

人话解释：一个解释 signal 是谁，一个解释 signal 文件里的列是什么。

例子：

- `signal_contract.md` 更像“这个 signal 的合同”
- `signal_fields_contract.md` 更像“这张 parquet 里的每列都代表什么”

二者一起用，才能让下游既知道身份，又知道 schema。

### `selected_symbols_test`

人话解释：在 TestEvidence 结束时，正式冻结下来、允许进入 backtest 的 symbol 白名单。
...
人话解释：TestEvidence 给 BacktestReady 的正式冻结规格。
...
> Backtest 之后不能再自己改白名单，也不能再自己挑一个更好看的 horizon。
