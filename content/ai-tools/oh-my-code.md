...
Oh My OpenCode 是 OpenCode 的插件，通过多 Agent 协作系统，将单一的 AI 编程助手升级为具备高度自主性的 AI 开发军团。
...
自动探索代码库模式
    ↓
并行调用专家 Agent (Hephaestus, Oracle, Librarian...)
    ↓
验证结果（测试、诊断）
    ↓
未完成？继续迭代
...
4. **确认计划**
   ...
   Prometheus 生成详细计划并保存到 `.sisyphus/plans/支付流程计划.md`
...
MCP 服务器不再占用上下文窗口。技能按需启动自己的 MCP，完成后自动清理。

### Todo Enforcer

Agent 空闲时，系统自动把它拉回任务，确保任务完成。

### Comment Checker

移除 AI 生成的冗余注释，保留有价值的说明。
...
// 之后 (经过 Comment Checker)
function add(a, b) { return a + b; }
```

### Tmux Integration

Agent 停留在会话中，支持所有交互式工具：REPLs (Python, Node, IRB)、Debuggers (gdb, lldb)、TUI Apps (htop, vim, tmux)。
...
`/init-deep` 遍历项目目录结构，在每个关键层级创建 `AGENTS.md` 文件：

```
项目根目录 AGENTS.md → 项目整体架构、技术栈、全局规范
        ↓
src/AGENTS.md → 模块职责、依赖关系、编码约定
        ↓
src/components/AGENTS.md → 组件设计模式、通信约定
```
...
适合新项目初始化、接手遗留项目、大型重构前、团队协作项目。

不适合简单单文件脚本、临时测试项目、个人熟悉的小项目。

**不同项目类型策略：**

- 前端项目：`/init-deep --max-depth=4`，重点关注 components、lib、hooks
- 后端项目：`/init-deep --max-depth=5`，重点关注 controllers、services、models、routes
- Monorepo：`/init-deep`，为每个 package 单独生成 AGENTS.md

### `/ulw-loop` — 持续工作循环

让 AI 持续工作直到任务 100% 完成。
...
```
Install and configure oh-my-opencode by following the installation guide
```
...
如果你符合按 Token 付费的条件，使用 Kimi 和 Gemini 模型成本更低。

### Agent-模型映射

| Agent | 推荐模型 | 特点 |
|-------|----------|------|
| Sisyphus | claude-opus-4-6 / kimi-k2.5 / glm-5 | 编排能力 |
| Hephaestus | gpt-5.3-codex | 代码实现 |
| Prometheus | claude-opus-4-6 / kimi-k2.5 / glm-5 | 规划思考 |
| Oracle | 高质量推理模型 | 架构分析 |
| Librarian | 便宜模型 | 文档搜索 |
| Explore | 免费模型 | 代码库搜索 |
