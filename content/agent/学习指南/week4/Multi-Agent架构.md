...
前文已经覆盖单 Agent 的基础认知和工具调用，今天要跨入一个新的层次：Multi-Agent，多智能体架构。
...
这篇文章的核心目标是理解 Multi-Agent 的价值、掌握分工和通信的设计方法、学会处理 Agent 间的冲突和结果整合。读完这一部分，你应该能设计出一个由 6 个 Agent 组成的行业研究团队，并定义每个 Agent 的职责边界和协作协议。
...
第二个痛点是"工具太多，选择困难"。前文讨论了 8 个工具，如果把全部 8 个工具给一个 Agent，它每次决策都要从 8 个工具中选择，选错概率不低。如果分成 6 个 Agent，每个 Agent 只配备 2-3 个工具，选择就简单多了。工具少了，决策准确率自然提高。
...
这里设计了一个能自动运行的 Multi-Agent 系统。但有一个问题：如果 Agent 做出了错误的判断，怎么办？如果它推荐了一个实际上不可行的方案，报告已经发出去了，怎么挽回？这就是 Human-in-the-loop 要解决的问题。下一篇会进入 Agent 系统中的人工审核和干预机制。
